                ...
                <div key={reply.id} className="flex items-start gap-2 p-2.5 rounded-lg" style={{ background: "var(--background)" }}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {reply.updated_at && (
                        <span className="text-xs" style={{ color: "var(--muted)" }}>· 已编辑于 {new Date(reply.updated_at).toLocaleString("zh-CN")}</span>
                      )}
                    </div>
                    {editingId === reply.id ? (
                      <div className="mt-1 space-y-1.5">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex gap-1.5">
                            <Button size="sm" onClick={() => handleEditSave(reply.id)}>保存</Button>
                            <Button size="sm" variant="secondary" onClick={() => { setEditingId(null); setEditImageFile(null); setEditImagePreview(null); }}>取消</Button>
                          </div>
                        </div>
                      </div>
                      ...
                      <>
                        ...
                        {reply.image_url && (
                          <img src={reply.image_url} alt="图片" className="mt-1.5 max-w-xs rounded-xl" />
                        )}
                        {reply.author_id === userId && (
                          <div className="flex gap-3 mt-1">
                            <button onClick={() => { setEditingId(reply.id); setEditContent(reply.content); setEditImageUrl(reply.image_url ?? null); setEditImageFile(null); setEditImagePreview(null); }} className="text-xs" style={{ color: "var(--muted)" }}>编辑</button>
                            <button onClick={() => handleDelete(reply.id)} className="text-xs" style={{ color: "#ef4444" }}>撤回</button>
                          </div>
                        )}
                      </>
                    )}
