...
export async function generateLessonPlan(input: string | LessonPlanInput): Promise<LessonPlan> {
  const normalized: LessonPlanInput =
    typeof input === "string"
      ? { knowledgePoint: input, difficulty: "review" }
      : { ...input, difficulty: input.difficulty ?? "review" };

  const difficultyMap: Record<NonNullable<LessonPlanInput["difficulty"]>, string> = {
    preview: "预习",
    review: "复习",
    advanced: "拔高",
  };
  const difficultyLabel = difficultyMap[normalized.difficulty ?? "review"];

  const learnerGoal =
    difficultyLabel === "预习"
      ? "重点做生动有趣的引入，帮助学生快速建立直觉，可附少量基础练习。"
      : difficultyLabel === "复习"
      ? "重点总结核心知识、易错点和常见陷阱，搭配难度适中的例题。"
      : "重点提供挑战型问题与高阶思路，鼓励举一反三。";

  const userPrompt = [
    `学习阶段：${difficultyLabel}`,
    normalized.subject ? `学科：${normalized.subject}` : "",
    normalized.textbook ? `教材：${normalized.textbook}` : "",
    normalized.chapter ? `章节：${normalized.chapter}` : "",
    normalized.knowledgePoint ? `知识点：${normalized.knowledgePoint}` : "",
    normalized.note ? `备注：${normalized.note}` : "",
    normalized.sourceText ? `参考正文材料：\n${normalized.sourceText}` : "",
    `生成要求补充：${learnerGoal}`,
  ]
    .filter(Boolean)
    .join("\n\n");
...
请严格按照以下 JSON 格式返回，不要有任何额外文字：
{
  ...
  "segments": [
    {
      "text": "该段朗读内容（140-220字，要求：①口语化、连贯、细节充分 ②必须包含可操作信息，不能只讲概念 ③涉及题目时必须讲清题干要点、思路、步骤和结论 ④适合TTS朗读，不用生僻符号堆砌；最后一段必须以'好，我们来复盘一下：'开头）",
...
- **段数根据知识点复杂度自然决定**，简单知识 4-5 段，复杂知识 6-9 段，禁止过短
- 最后一段必须是整课总结，以"好，我们来复盘一下："开头

【必须包含“例题/案例 + 分析过程”】
- 若学科是数学/物理/化学：至少 1 道完整例题，必须包含“题干条件→建模/公式选择→逐步推导→答案→易错点提醒”
- 若学科是语文/英语/政治/历史/地理/生物：至少 1 个典型案例或题型，必须包含“题干或材料关键信息→解题/分析步骤→结论→迁移方法”
- 若是编程/信息技术：至少 1 个具体场景示例，必须包含“输入条件→思路→关键步骤/伪代码→输出结果”
- 例题或案例内容必须在 text 中详细出现，不可只放在 visualItems

【不同学科的表达策略（自动匹配）】
- 理科：强调条件、公式来源、推导链路、单位/边界检查
- 文科：强调背景、观点依据、关键词解释、答题模板
- 语言学科：强调语境、语法点、常见误区、可复用表达
- 技术学科：强调步骤、输入输出、异常情况、调试思路

【复盘友好要求】
- 每段结尾补一句“这一段你要记住：...”
- 总结段必须给出“3-5条复盘清单”（可直接用于考前回看）
- 总结段再给“1条自测题 + 简短参考答案”
...
4. **均匀覆盖全文**：5 张卡 → 每张覆盖约 1/5 text；6 张 → 约 1/6；保证卡片出现时机与字幕高度吻合
5. 每段 5-6 张卡片，类型尽量多样
...
- 若该段包含例题/案例，必须包含一个 steps 卡片，步骤数不少于 4 步
